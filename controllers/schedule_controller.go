...
// ViewTeacherSchedule returns the authenticated teacher's complete weekly
// timetable. Unlike /teacher/classes, each row identifies the exact course,
// room and time slot so the web can show current and upcoming lessons.
func ViewTeacherSchedule(c *gin.Context) {
	teacher, ok := getCurrentTeacher(c)
	if !ok {
		return
	}

	var schedules []responses.TeacherScheduleResponse
	err := config.DB.Table("schedules s").
		Select(`
			s.id,
		...
		`).
		Joins("JOIN course_offerings co ON co.id = s.course_offering_id").
		Joins("JOIN classes cl ON cl.id = co.class_id").
		Joins("JOIN majors m ON m.id = cl.major_id").
		Joins("JOIN courses c ON c.id = co.course_id").
		Joins("LEFT JOIN rooms r ON r.id = co.room_id").
		Joins("JOIN semesters sem ON sem.id = co.semester_id").
		Where("co.teacher_id = ? AND co.status = ?", teacher.ID, "open").
		Order("FIELD(s.day_of_week, 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'), s.start_time, cl.class_code, c.code").
		Scan(&schedules).Error
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": "Lỗi lấy lịch dạy", "error": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{
		"message": "Lấy lịch dạy giảng viên thành công",
		"data":    schedules,
	})
}
