...
func ListTeacherClasses(c *gin.Context) {
	...
	if err := config.DB.Preload("Major").Preload("Room").Preload("Semester").
		Preload("Schedules").
		Preload("CourseOfferings", "teacher_id = ? AND status = ?", teacher.ID, "open").
		Preload("CourseOfferings.Course").
		Preload("CourseOfferings.Room").
		Preload("CourseOfferings.Semester").
		Preload("CourseOfferings.Schedules").
...
func ListClassGrades(c *gin.Context) {
	...
	if err := config.DB.Preload("Student.User").Preload("Course").Preload("Grade").
		...
		Where("enrollments.class_id = ? AND course_offerings.teacher_id = ?", classID, teacher.ID).
...
func UpsertGrade(c *gin.Context) {
	...
	if err := config.DB.Preload("Class").Preload("CourseOffering").First(&enrollment, req.EnrollmentID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"message": "Không tìm thấy đăng ký học phần"})
		return
	}

	if !teacherOwnsEnrollment(enrollment, teacher.ID) {
...
func UpdateGrade(c *gin.Context) {
	...
	var grade models.Grade
	if err := config.DB.Preload("Enrollment.Class").Preload("Enrollment.CourseOffering").First(&grade, gradeID).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"message": "Không tìm thấy điểm"})
		return
	}

	if !teacherOwnsEnrollment(grade.Enrollment, teacher.ID) {
...
func ApproveClassGrades(c *gin.Context) {
	...
	var enrollments []models.Enrollment
	if err := config.DB.Preload("Grade").
		Joins("JOIN course_offerings ON course_offerings.id = enrollments.course_offering_id").
		Where("enrollments.class_id = ? AND course_offerings.teacher_id = ?", classID, teacher.ID).
		Find(&enrollments).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": "Lỗi lấy danh sách điểm", "error": err.Error()})
...
func teacherOwnsClass(classID uint, teacherID uint) bool {
	var count int64
	config.DB.Model(&models.CourseOffering{}).Where("class_id = ? AND teacher_id = ?", classID, teacherID).Count(&count)
	if count == 0 {
...
func teacherOwnsEnrollment(enrollment models.Enrollment, teacherID uint) bool {
	if enrollment.CourseOfferingID != nil {
		if enrollment.CourseOffering != nil && enrollment.CourseOffering.ID != 0 {
			return enrollment.CourseOffering.TeacherID == teacherID
		}
		var count int64
		config.DB.Model(&models.CourseOffering{}).
			Where("id = ? AND teacher_id = ?", *enrollment.CourseOfferingID, teacherID).
			Count(&count)
		return count > 0
	}
	return enrollment.Class.TeacherID == teacherID
}
