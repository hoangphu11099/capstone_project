...
func CreateClassOffer(c *gin.Context) {
	...
	// Cảnh báo sớm nếu giảng viên đã trùng lịch với lớp này
	var classSchedules []models.Schedule
	if err := config.DB.Where("class_id = ?", req.ClassID).Find(&classSchedules).Error; err == nil {
		if err := checkTeacherScheduleConflict(req.TeacherID, req.ClassID, classSchedules); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"message": "Không thể đề xuất vì giảng viên bị trùng lịch với lớp này", "error": err.Error()})
			return
		}
	}
...
func respondClassOffer(c *gin.Context, status string) {
	...
	// Kiểm tra trùng lịch trước khi chấp nhận
	if status == "accepted" {
		var classSchedules []models.Schedule
		if err := config.DB.Where("class_id = ?", offer.ClassID).Find(&classSchedules).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"message": "Lỗi lấy lịch học của lớp", "error": err.Error()})
			return
		}
		if err := checkTeacherScheduleConflict(teacher.ID, offer.ClassID, classSchedules); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"message": "Không thể chấp nhận vì giảng viên bị trùng lịch", "error": err.Error()})
			return
		}
	}
	now := time.Now()
