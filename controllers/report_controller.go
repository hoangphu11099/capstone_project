...
func gradeSummaryByScope(teacherID uint) gin.H {
	...
	baseQuery().Select("AVG(COALESCE(NULLIF(g.total_score, 0), g.score, 0)) AS average").Scan(&avg)

	var approved int64
	var draft int64
	baseQuery().Where("LOWER(g.status) = ?", "approved").Count(&approved)
	baseQuery().Where("LOWER(g.status) = ?", "draft").Count(&draft)

	letterCounts := map[string]int64{}
	letters := []string{"A", "B+", "B", "C+", "C", "D+", "D", "F"}
	for _, letter := range letters {
		var count int64
		baseQuery().Where("COALESCE(NULLIF(g.grade_letter, ''), g.letter_grade, '') = ?", letter).Count(&count)
