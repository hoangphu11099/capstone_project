...
func main() {
	...
	r := gin.Default()
	// ===== CORS =====
	// Có thể cấu hình nhiều origin bằng CORS_ORIGINS, phân tách bằng dấu phẩy.
	r.Use(func(c *gin.Context) {
		origin := c.GetHeader("Origin")
		allowedOrigins := strings.TrimSpace(os.Getenv("CORS_ORIGINS"))
		allowOrigin := ""
		if origin != "" {
			if allowedOrigins == "" {
	...
	r.GET("/schedule-test", controllers.ViewSchedule)
	...
	auth.Use(middleware.AuthRequired())
	{
		...
		auth.POST("/classes/:id/course-offerings", middleware.RoleRequired("admin"), controllers.AssignCourseOffering)
		...
		auth.GET("/teacher/schedule", middleware.RoleRequired("teacher"), controllers.ViewTeacherSchedule)
		auth.GET("/classes/:id/grades", middleware.RoleRequired("teacher"), controllers.ListClassGrades)
		...
		auth.POST("/classes/:id/grades/approve", middleware.RoleRequired("teacher"), controllers.ApproveClassGrades)
		...
		// Dữ liệu tham chiếu cho dropdown/form web dashboard.
		auth.GET("/metadata", middleware.RoleRequired("teacher"), controllers.GetMetadata)
	}
	...
	auth.GET("/attendance/sessions", middleware.RoleRequired("teacher"), controllers.ListAttendanceSessions)
	...
	auth.POST("/attendance/sessions/:id/close", middleware.RoleRequired("teacher"), controllers.CloseAttendanceSession)
	...
	auth.PUT("/submissions/:id/grade", middleware.RoleRequired("teacher"), controllers.GradeSubmission)
